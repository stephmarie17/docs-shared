...
- Performance improvements due to implementation of native cryptography
  ...
  - **Windows**: Upgrade to your ``mongodb-crypt`` version to v5.2.0.

  - **Mac**: Upgrade to your ``mongodb-crypt`` version to v5.2.0.

  - **Linux**: Install ``libmongocrypt.so`` directly on the file system,
    instead of using the file that is bundled within the
    ``mongodb-crypt`` JAR file. View Linux instructions for installing
    ``libmongocrypt`` in the :manual:`Server manual
    </core/csfle/reference/libmongocrypt/#linux-installation>`. If you
    use a package manager to install ``libmongocrypt``, JNA will find it
    there without further configuration. If there is an issue, you can
    specify the search path by setting ``LD_LIBRARY_PATH``.
